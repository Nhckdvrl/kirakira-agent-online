...
from agent.control.protocol.models import (
    METHOD_PARAMS,
    InitializeParams,
...
class ConnectionRouter:
    ...
    async def _dispatch(self, request: JsonObject) -> object:
        ...
        try:
            values = model_type.validate(raw_params)
        except ParamValidationError as exc:
            raise JsonRpcError(
                INVALID_PARAMS, "Invalid params", {"issues": exc.issues}
            ) from exc
        ...
        if method == "initialize":
            ...
            result = self._service.initialize(InitializeParams.from_values(values))
        ...
        return await self._call_method(method, values)

    async def _call_method(self, method: str, values: dict[str, Any]) -> object:
