"""Agent 自请求重启的协调器(照 Reference `agent/restart.py` 移植)。

supervisor 一侧的握手(`agent/supervisor.py`,与 Reference 逐字节一致)早已在等:
私有管道 fd、boot 一次性 nonce、退出码 75。本模块补进程内的另一半——
`agent_restart` 工具 arm 一个请求,冻结新 turn 准入,等 caller turn 正式落终态
**且**最终响应真实送达后,才向管道写入唯一一帧提交证据并让 gateway 以 75 退出。
任一环节失败(turn 非 completed、送达失败、15s watchdog 超时)都恢复准入,
supervisor 不会看到有效提交,也就不会误换代。

与 Reference 的差异只有 `from_environment` 同时接受 `AKASHIC_*` 与 `KIRAKIRA_*`
环境变量名(与 cli.py 的 readiness 识别同口径)。
"""
...
def _env(name: str, default: str = "") -> str:
    """双读 AKASHIC_* 与 KIRAKIRA_*;supervisor 是 Reference 原文,设的是 AKASHIC_*。"""
    value = os.environ.get("AKASHIC_%s" % name)
    if value is None:
        value = os.environ.get("KIRAKIRA_%s" % name)
...
class SupervisorCommitChannel:
    ...
    def from_environment(cls) -> "SupervisorCommitChannel | None":
        supervised = _env("SUPERVISED") == "1"
        if not supervised:
            return None
        raw_fd = _env("RESTART_COMMIT_FD", default="")
        boot_id = _env("BOOT_ID")
        nonce = _env("RESTART_NONCE")
        if not raw_fd:
