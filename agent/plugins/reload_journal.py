"""Persistent transaction journal for plugin generation replacement.

This is the Kirakira adapter of Reference ``agent/plugins/reload_journal.py``.
The journal is intentionally independent from the in-memory plugin manager so a
new process can make a deterministic recovery decision before publishing any
...
class ReloadJournal:
    """Persist plugin reload phases and expose crash-recovery work."""

    def __init__(self, workspace: Path) -> None:
        self.path = workspace / ".kirakira" / "runtime" / "plugin-reloads.sqlite3"
    ...
    def advance(
        ...
        *,
        ...
        with self._connect() as conn:
            row = conn.execute(
                "SELECT phase FROM reload_transactions WHERE tx_id = ?", (tx_id,)
            ).fetchone()
            if row is None:
        ...
        with self._connect() as conn:
            row = conn.execute(
                ...
                SELECT tx_id, plugin_id, base_snapshot_id, candidate_snapshot_id,
                       generation_id, source_revision, config_revision, phase,
                       started_at, updated_at, error
                FROM reload_transactions WHERE tx_id = ?
                """,
                (tx_id,),
            ).fetchone()
        if row is None:
    ...
    def events(self, tx_id: str) -> tuple[ReloadJournalEvent, ...]:
        with self._connect() as conn:
            rows = conn.execute(
                ...
                FROM reload_events WHERE tx_id = ? ORDER BY sequence
                """,
                (tx_id,),
            ).fetchall()
        return tuple(
...
def _record(row: tuple[object, ...]) -> ReloadTransactionRecord:
