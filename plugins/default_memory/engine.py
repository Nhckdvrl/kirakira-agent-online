...
from infra.providers.model_client_adapter import LLMProvider, LLMResponse
from agent.skill_loader import SkillsLoader
...
from memory2.persistence import MemoryPersistence
...
class DefaultMemoryEngine:
    ...
    def __init__(
        ...
        *,
        ...
        memory_persistence: MemoryPersistence | None = None,
    ) -> None:
        self._config = config
        ...
        self._v2_store: MemoryStore2 | MemoryPersistence | None = None
        ...
        self._v2_store = memory_persistence or MemoryStore2(
            db_path,
        ...
        self.closeables = [
            owner
            for owner in (self._v2_store, self._embedder)
            if callable(getattr(owner, "close", None))
        ]
    ...
    def _on_turn_committed(self, event: TurnCommitted) -> None:
        ...
        self._event_bus.enqueue(
            TurnIngested(
                ...
                user_id=str((event.extra or {}).get("principal_id") or ""),
            )
        )
    ...
    async def _on_turn_ingested(self, event: TurnIngested) -> None:
        ...
        with self._bind_user_scope(scope):
            await self._post_response_worker.handle(event)

    async def _on_consolidation_committed(
        self,
        event: ConsolidationCommitted,
    ) -> None:
        with self._bind_user_scope(MemoryScope(user_id=event.user_id)):
            await self._run_consolidation_committed(event)

    async def _run_consolidation_committed(
        self,
        event: ConsolidationCommitted,
    ) -> None:
    ...
    def undo_by_message_sources(
        ...
        *,
        ...
        store = self._require_v2_store()
        cloud_undo = getattr(store, "undo_by_message_sources", None)
        if callable(cloud_undo):
            return cast(
                dict[str, object],
                cloud_undo(message_ids, dry_run=dry_run),
            )
    ...
    def _bind_user_scope(
        ...
        if not user_id:
            raise RuntimeError("Cloud memory operation requires MemoryScope.user_id")
        return cast(AbstractContextManager[object], binder(user_id))

    def _require_v2_store(self) -> MemoryStore2 | MemoryPersistence:
