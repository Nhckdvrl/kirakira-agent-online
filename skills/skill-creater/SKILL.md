...
```
<workspace>/skills/{skill-name}/   ← 用户自定义技能（优先）
  SKILL.md                         ← 必须
  scripts/                         ← 可选，放辅助脚本
  references/                      ← 可选，放参考文档
...
1. 确认技能名称和保存位置（workspace 还是内置项目 `skills/`）
2. 用 `write_file` 写入 `<目标目录>/skills/<skill-name>/SKILL.md`
3. 如有辅助脚本，写入 `scripts/` 目录
4. 验证：`requires.bins` 里的命令是否正确；用 `/skills` 或重新加载确认它出现在清单里
...
- 安装命令是否适配当前系统（macOS 用 `brew`，Linux 用 `uv`/`apt`/`pacman`）
